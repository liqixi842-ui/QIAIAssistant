...
- **Key Backend Features**:
    ...
    - **WhatsApp Chat History Upload** (Oct 28, Enhanced Oct 29, Oct 31):
        - **Parser**: `parseWhatsAppChat()` function supports both `[DD/MM/YY HH:MM:SS] Sender: Message` AND `[DD/MM/YY HH:MM:SS] Sender - Message` formats with improved regex `([:|-])`, normalizes all line break formats (\r\n, \n, \r), handles multi-line messages, preserves hyphenated names (e.g., "Sophie-Miller"), filters system messages, returns array of {timestamp, sender, message} objects.
        - **Manual Role Assignment** (Oct 31): Primary method for role identification - users input their WhatsApp display name during upload, system assigns roles via exact string matching (sender === agentName → agent, else → customer). This eliminates AI misidentification issues and provides 100% accuracy. Frontend validation ensures agentName is required before submission.
        - **AI Role Detection**: `identifyRolesWithAI()` fallback method (used only when agentName not provided for backward compatibility) analyzes conversations to distinguish customer vs agent messages based on tone and content, with schema validation to prevent prompt injection attacks.
        - **API**: `POST /api/customers/:id/upload-chat` receives chatText and optional agentName parameters; if agentName provided, uses manual role assignment (faster, more accurate); otherwise falls back to AI identification. Request body limit increased to 10MB to handle large chat histories.
        - **Frontend**: Upload button in Customer Details dialog (Conversation tab), modal with required "业务员姓名" input field (with helper text), textarea for pasting chat history OR file upload button for .txt files, submit button disabled when agentName empty, displays imported conversations as individual message bubbles with role-based styling (agent: right-aligned primary, customer: left-aligned muted).
